    ...
    return (
        <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map(i => (
                <Star
                    key={i}
                    className={cn(s, i <= rating ? 'text-amber-400 fill-amber-400' : 'text-slate-600')}
                />
            ))}
        </div>
    );
...
const RatingBar = ({ rating, count, total }: { rating: number; count: number; total: number }) => (
    <div className="flex items-center gap-3">
        <span className="text-xs text-slate-400 font-bold w-4">{rating}</span>
...
export default function TutorReviewsPage() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            {/* Header */}
            <div>
                <p className="text-amber-400 text-sm font-bold uppercase tracking-widest mb-1">Feedback</p>
                <h1 className="text-4xl font-extrabold text-white mb-2 tracking-tight">Reviews & Ratings</h1>
                <p className="text-slate-400">See what your students say about you.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Rating Overview */}
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                    <h2 className="text-lg font-bold text-white mb-5">Rating Overview</h2>
                ...
                <div className="lg:col-span-2 space-y-4">
                    {loading ? (
                        ...
                        <div className="text-center py-20 bg-white/5 border border-white/10 rounded-2xl">
                            ...
                            <p className="text-slate-400">Complete sessions to start receiving student reviews.</p>
                        </div>
                    ) : (
                        reviews.map((review) => (
                            <div key={review._id} className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-all">
                                <div className="flex items-start gap-4 mb-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center justify-between">
                                            <h3 className="font-bold text-white">{review.student.fullName}</h3>
                                ...
                                {review.comment && (
                                    <p className="text-slate-300 text-sm leading-relaxed pl-14">"{review.comment}"</p>
                                )}
            ...
            <div className="bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 rounded-2xl p-6">
                <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-white">Tips to Improve Your Profile</h2>
                        <p className="text-xs text-slate-400">Follow these to attract more students</p>
                    </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {tips.map((tip, i) => (
                        <div key={i} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl">
                            <div className="w-6 h-6 rounded-full bg-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-xs shrink-0 mt-0.5">
                                {i + 1}
                            </div>
