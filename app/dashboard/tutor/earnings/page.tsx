    ...
    <div className="flex items-end gap-2 h-32">
        {data.map((d, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                <span className="text-xs text-slate-400 font-bold">Rs.{d.value > 999 ? `${(d.value / 1000).toFixed(1)}k` : d.value}</span>
                <div className="w-full flex flex-col justify-end" style={{ height: '80px' }}>
                    <div
                        className="w-full bg-gradient-to-t from-indigo-600 to-indigo-400 rounded-t-lg hover:from-indigo-500 hover:to-indigo-300 transition-all duration-300"
...
export default function TutorEarningsPage() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            {/* Header */}
            <div>
                <p className="text-emerald-400 text-sm font-bold uppercase tracking-widest mb-1">Tutor Finance</p>
                <h1 className="text-4xl font-extrabold text-white mb-2 tracking-tight">Earnings Dashboard</h1>
                <p className="text-slate-400">Track your income, pending payments, and withdrawals.</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {[
                    ...
                    { label: 'Completed', value: completedTx.length.toString(), icon: CheckCircle, color: 'from-blue-500 to-indigo-600', sub: 'Transactions' },
                    { label: 'Avg per Session', value: completedTx.length > 0 ? `Rs. ${Math.round(totalEarnings / completedTx.length)}` : 'Rs. 0', icon: TrendingUp, color: 'from-purple-500 to-pink-600', sub: 'Per booking' },
                ].map((card, i) => (
                    <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6 relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
                        ...
                        <div className="relative z-10">
                            ...
                            <p className="text-2xl font-black text-white mb-0.5">{loading ? '...' : card.value}</p>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 text-slate-500">
                        ...
                        <p className="font-medium text-white mb-1">No earnings yet</p>
                        ...
                        {transactions.map((tx) => (
                            <div key={tx._id} className="flex items-center gap-4 p-4 bg-white/5 border border-white/5 rounded-xl hover:border-white/10 transition-all">
                                ...
                                <div className="text-right shrink-0">
                                    <p className="font-black text-emerald-400">+Rs. {tx.amount.toLocaleString()}</p>
