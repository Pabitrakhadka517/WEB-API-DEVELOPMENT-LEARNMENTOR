            ...
            <div className="min-h-screen bg-[#0b0f1a] p-8">
                <div className="max-w-2xl mx-auto text-center space-y-6">
                    <div className="p-8 bg-white/5 border border-white/10 rounded-3xl">
                        ...
                        <p className="text-slate-400 mt-2">{error || "The user you're looking for doesn't exist."}</p>
                    </div>
                    <button onClick={() => router.push('/admin')} className="inline-flex items-center gap-2 text-indigo-400 font-bold hover:text-indigo-300 transition-colors">
        ...
        <div className="min-h-screen bg-[#0b0f1a] text-white p-8">
            <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <button onClick={() => router.push('/admin')} className="group flex items-center gap-3 text-slate-400 hover:text-white transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => router.push(`/admin/users/${id}/edit`)} className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold text-sm transition-all flex items-center gap-2 shadow-lg shadow-indigo-600/20">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-6">
                        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 text-center space-y-6">
                            <div className="relative inline-block">
                                <div className="w-32 h-32 rounded-3xl bg-indigo-600/20 border-2 border-indigo-500/40 flex items-center justify-center text-4xl font-black text-indigo-400 overflow-hidden mx-auto shadow-2xl">
                                    {user.profileImage ? (
                                        <img src={user.profileImage} alt={user.name} className="w-full h-full object-cover" />
                                    ) : user.name[0].toUpperCase()}
                                </div>
                                <div className={cn(
                                    "absolute -bottom-2 -right-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-xl",
                            ...
                            <div>
                                ...
                                <p className="text-indigo-400 font-bold text-sm uppercase tracking-widest mt-1">{user.role}</p>
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h3 className="font-black text-white uppercase tracking-widest text-xs">Primary Address</h3>
                                </div>
                                <p className="text-slate-400 font-medium leading-relaxed">{user.address || 'Not provided'}</p>
                            </div>
                            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-4">
                                <div className="flex items-center gap-3">
                                    <Calendar className="w-5 h-5 text-indigo-400" />
                                    <h3 className="font-black text-white uppercase tracking-widest text-xs">Account Created</h3>
                                </div>
                                <p className="text-slate-400 font-medium">{new Date(user.joined).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                            </div>
                        </div>
                        ...
                        {user.role === 'TUTOR' && (
                            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 space-y-8">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-xl font-black text-white">Tutor Profile Settings</h3>
                                    <div className={cn(
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-lg font-bold text-white">{user.speciality}</p>
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex items-center gap-2 text-slate-500 mb-2">
                                ...
                                {user.tutorProfile?.bio && (
                                    <div className="pt-6 border-t border-white/5">
                                        <div className="flex items-center gap-2 text-slate-500 mb-3">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest">Professional Bio</span>
                                        </div>
                                        <p className="text-slate-400 leading-relaxed italic">"{user.tutorProfile.bio}"</p>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        <div className="bg-gradient-to-br from-indigo-600/10 to-purple-600/10 border border-white/10 rounded-3xl p-8">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-lg font-black text-white">Recent Activity Log</h3>
                                <button className="text-xs font-black text-indigo-400 hover:text-indigo-300 uppercase tracking-widest">Full History</button>
                            </div>
